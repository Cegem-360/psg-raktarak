<x-layouts.app>
    <x-slot name="title">{{ $post->title }} | PSG-RAKTÁRAK Blog</x-slot>
    <x-slot name="meta">
    ...
    <div class="min-h-screen bg-gray-50">
        ...
        <article class="max-w-4xl px-4 py-12 mx-auto sm:px-6 lg:px-8">
            <!-- Article Header -->
            <header class="mb-8">
                ...
                <div class="flex items-center justify-between pb-6 text-sm text-gray-600 border-b">
                    <div class="flex items-center space-x-4">
                        <div class="flex items-center">
                            <div
                                class="flex items-center justify-center w-10 h-10 font-medium text-white rounded-full bg-gradient-to-r from-blue-500 to-blue-600">
            ...
            <div class="prose prose-lg max-w-none">
                @if ($post->excerpt)
                    <div class="p-6 mb-8 border-l-4 border-blue-400 bg-blue-50">
                        <p class="text-lg italic font-medium text-blue-800">{{ $post->excerpt }}</p>
                    </div>
                @endif
            ...
            <footer class="pt-8 mt-12 border-t">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="mb-2 text-sm text-gray-500">Kategória:</p>
                        <a href="{{ localized_route('blog.category', ['slug' => $post->category->slug]) }}"
                            class="inline-flex items-center px-3 py-1 text-sm font-medium text-white transition duration-150 rounded-full hover:opacity-90"
                            style="background-color: {{ $post->category->color }}">
                            {{ $post->category->name }}
                        </a>
                    </div>
                    <div class="text-right">
                        <p class="mb-2 text-sm text-gray-500">Megosztás:</p>
                        <div class="flex space-x-2">
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(Request::url()) }}"
                                target="_blank"
                                class="p-2 text-white transition duration-150 bg-blue-600 rounded-lg hover:bg-blue-700">
                            ...
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(Request::url()) }}&text={{ urlencode($post->title) }}"
                                target="_blank"
                                class="p-2 text-white transition duration-150 bg-blue-400 rounded-lg hover:bg-blue-500">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                            ...
                            <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(Request::url()) }}"
                                target="_blank"
                                class="p-2 text-white transition duration-150 bg-blue-700 rounded-lg hover:bg-blue-800">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
        ...
        @if ($relatedPosts->count() > 0)
            <section class="py-16 bg-white border-t">
                <div class="max-w-6xl px-4 mx-auto sm:px-6 lg:px-8">
                    <h2 class="mb-8 text-2xl font-bold text-gray-900">Kapcsolódó cikkek</h2>
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                        @foreach ($relatedPosts as $relatedPost)
                            <article
                                class="overflow-hidden transition duration-300 rounded-lg bg-gray-50 hover:shadow-md">
                                @if ($relatedPost->featured_image)
                                    <div class="aspect-w-16 aspect-h-9">
                                        <img src="{{ Storage::url($relatedPost->featured_image) }}"
                                            alt="{{ $relatedPost->title }}" class="object-cover w-full h-48"
                                            loading="lazy">
                                    </div>
                                @else
                                    <div
                                        class="flex items-center justify-center w-full h-48 bg-gradient-to-br from-gray-200 to-gray-300">
